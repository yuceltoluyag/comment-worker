...
app.post("/api/handle/form", async (c) => {
  ...
  const gh = await GitHub.initialize(
    appId,
    formattedPrivateKey,
    organizationSlug,
    repositorySlug,
  ...
  const staticmanFile = await gh.getFileFromRepository(
    "staticman.yml",
    repositoryBranch,
  );
  ...
  if (contentTypeHeader === "application/x-www-form-urlencoded") {
    ...
    if (shouldDebug)
      console.log("Form URL-encoded body:", JSON.stringify(body, null, 2));
  } else if (contentTypeHeader === "application/json") {
    body = await req.json();
    if (shouldDebug) console.log("JSON body:", JSON.stringify(body, null, 2));
  } else {
    return c.text("Unsupported Content-Type", 400);
  }

  // Check if body has direct fields instead of fields/options structure
  // and restructure it if needed
  if (
    body.name !== undefined ||
    ...
    if (shouldDebug)
      console.log(
        "Form doğrudan değerler içeriyor, yeniden yapılandırılıyor...",
      );

    // Extract values that should be in fields
    const newFields = {};
    staticmanCommentsConfig?.allowedFields?.forEach((field) => {
      if (body[field] !== undefined) {
        newFields[field] = body[field];
      }
    });

    // Extract values that should be in options
    const newOptions = {};
    staticmanCommentsConfig?.allowedOptions?.forEach((option) => {
      if (body[option] !== undefined) {
        newOptions[option] = body[option];
      }
    });
  ...
  if (shouldDebug) {
    ...
    console.log(
      ...
      env.TURNSTILE_SECRET_KEY ? "Mevcut" : "Eksik",
    );
  ...
  if (!validCaptcha) {
    if (shouldDebug) console.log("Turnstile doğrulaması başarısız!");
    return c.text("Invalid reCaptcha", 400);
  }

  if (shouldDebug) console.log("Turnstile doğrulaması başarılı!");
  ...
  if (shouldDebug) {
    ...
    console.log("Fields:", JSON.stringify(fieldValues, null, 2));
    console.log("Options:", JSON.stringify(optionValues, null, 2));
    console.log("allowedFields:", staticmanCommentsConfig?.allowedFields);
    console.log("allowedOptions:", staticmanCommentsConfig?.allowedOptions);
  }
  ...
  const fieldTransforms =
    staticmanCommentsConfig?.transforms ||
    staticmanCommentsConfig?.fieldTransforms ||
    {};
  ...
  const optionInputSchema = z
    .object(
      buildSchemaObject(allowedOptions, requiredOptions, optionTransforms),
    )
    .strict();
  ...
  const commitMessage = Object.prototype.hasOwnProperty.call(
    ...
    ? handlePlaceholders(
        staticmanCommentsConfig.commitMessage,
        fields,
        validatedOptions,
      )
    : "Add comment-worker data";
  const filename = Object.prototype.hasOwnProperty.call(
    staticmanCommentsConfig,
    "filename",
  )
    ? handlePlaceholders(
        staticmanCommentsConfig.filename,
        fields,
        validatedOptions,
      )
    : "";
  const directoryPath = Object.prototype.hasOwnProperty.call(
    staticmanCommentsConfig,
    "path",
  )
  ...
  if (moderation) {
    const createBranchResponse = await gh.createBranchOnRepository(
      branch,
      defaultBranch,
    );
  }
  ...
  if (!createCommentFileResponse?.content) {
    return c.text(
      "Failed to create comment file. Please check if your file path is valid.",
      422,
    );
  }
  ...
  if (moderation) {
    const pullRequestBody = Object.prototype.hasOwnProperty.call(
      ...
      ? handlePlaceholders(
          staticmanCommentsConfig.pullRequestBody,
          fields,
          validatedOptions,
        )
...
app.all(
  "*",
  () =>
    new Response("These are not the droids you are looking for", {
      status: 404,
    }),
);
